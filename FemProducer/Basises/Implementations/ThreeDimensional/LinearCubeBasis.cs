...
public class LinearCubeBasis : AbstractBasis
{
    ...
    public LinearCubeBasis(ProblemService problemService) : base(problemService) => _boundaryBasis = new LinearRectangularBasis(problemService);
    ...
    public override IList<IList<double>> GetMassMatrix(FiniteElement finiteElement)
    {
        ...
        for (var i = 0; i < NodesCountInElement; i++)
        for (var j = 0; j < NodesCountInElement; j++)
    ...
    public override Dictionary<string, IList<IList<double>>> GetLocalMatrixes(FiniteElement finiteElement) => new Dictionary<string, IList<IList<double>>>
    {
        {
    ...
    public static double GetValueInElementCenter(FiniteElement finiteElement, double[] valuesInNodes) => valuesInNodes[0];
}