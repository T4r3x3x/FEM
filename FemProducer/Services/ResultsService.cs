...
public class ResultsService<TLogger> where TLogger : ILogger
{
    ...
    public void PrintSlae(Slae slae)
    {
        ...
        for (var i = 0; i < matrix.Length; i++)
        {
            for (var j = 0; j < matrix.Length; j++)
            {
                if (matrix[i][j] >= 0)
                    Console.Write(" ");
                Console.Write($"{matrix[i][j]:E2} ");
            }
            Console.Write($" {slae.Vector[i]:E2}");
            Console.WriteLine();
        }
        Console.WriteLine("\n\n");
    }
    ...
    {
        var exactSolution = _resultProducer.AnalyticsSolves[timeLayer];
    ...
    public void WriteSolveWithGrid(string path, Vector solve)
    {
        ...
        using (var sw = new StreamWriter(file))
        {
            ...
            for (var i = 0; i < _grid.Nodes.Count; i++)
            {
                sw.Write($"{_grid.Nodes[i]} ");
                sw.Write($"{solve[i]} \n");
            }
        }
    }
    ...
    public void WriteSolve(string path, IList<Vector> solve)
    {
        ...
        using (var sw = new StreamWriter(file))
        {
            sw.WriteLine(solve[0].Length / _grid.Z.Count); //nodes count in plane
